...
class BumpAndGoController(Node):
    def __init__(self):
        ...
        self.laser_data_subscriber=self.create_subscription(LaserScan,"/scan", self.get_laser_data_callback, qos_profile_sensor_data)
    ...
    def get_laser_data_callback(self,laser_msg):
        ...
        # Divide the laser data into 12 sectors and compute mean distances
        self.laser_data_split = np.array_split(self.laser_data_limited, self.n_sectors)
        self.sectors_mean_distances = [np.mean(part) for part in self.laser_data_split]    # mean distance for each sector
    ...
    def control_loop_callback(self):
        ...
        idx20 = int((np.deg2rad(20) - self.angle_min) / self.angle_increment)   # index of the element corresponding to +20°
        idx40 = int((np.deg2rad(40) - self.angle_min) / self.angle_increment)   # index of the element corresponding to +40°

        distFR = np.mean(self.laser_data_limited[0:idx20])       # [0°, -20°], front right
        distR = np.mean(self.laser_data_limited[idx20:idx40])    # [-20°, -40°], front right
        distFL = np.mean(self.laser_data_limited[-idx20:])       # [0°, +20°], front left
        distL = np.mean(self.laser_data_limited[-idx40:-idx20])  # [+20°, +40°], front left
        ...
        if self.state != 'TURN' and (distFR < self.distance_limit or distR < self.distance_limit or distFL < self.distance_limit or distL < self.distance_limit):
            ...
            sectors_per_side = 4    # num of considered sectors on each side (approximately 4 * 30° = 120°)
            front_sectors = np.concatenate((self.sectors_mean_distances[:sectors_per_side], self.sectors_mean_distances[-sectors_per_side:]))
