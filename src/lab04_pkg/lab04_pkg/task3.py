...
class EKF_ESTIMATE(Node):
    def __init__(self):
        ...
        self.sigma_z = np.array([0.8, math.pi/10])
        ...
        self.w=0.0
        self.dt=0.05
        self.alpha = [0.001, 0.001, 0.0001, 0.0001, 0.001, 0.001]

        self.landmarks = {
            0: {"x": 1.20, "y":  1.68, "z": 0.16 },
            1: {"x": 1.68, "y": -0.05, "z": 0.18 },
            2: {"x": 3.72, "y":  0.14, "z": 0.22 },
            3: {"x": 3.75, "y":  1.37, "z": 0.21 },
            4: {"x": 2.48, "y":  1.25, "z": 0.22 },
            5: {"x": 4.80, "y":  1.87, "z": 0.24 },
            6: {"x": 2.18, "y":  1.00, "z": 0.24 },
            7: {"x": 2.94, "y":  2.70, "z": 0.73 },
            }
    ...
    def landmark_callback(self, measures: LandmarkArray):
        if measures.landmarks is None or len(measures.landmarks) == 0:
            self.get_logger().warn(f"----> NO LANDMARKS")

        for element in measures.landmarks:
            self.get_logger().info(f'Ricevuti {len(measures.landmarks)} landmarks')e 

            z=np.array([[element.range], [element.bearing]])
            id=element.id

            self.get_logger().warn(f"ID: {id}, RANGE: {z[0]}, BEARING: {z[1]}")
